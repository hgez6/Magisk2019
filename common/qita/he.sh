 ...
 if $FUNCTION; then  
    ...
    ui_print "音量－:触发是直的"
     ...
     h="1"
         ...
         ui_print " mi9游戏工具箱高级设置版弯触发安装成功"
		echo -n  " [mi9游戏工具箱高级弯]" >>$INSTALLER/module.prop
			...
			cp -f $INSTALLER/common/SecurityCenter.apk ${MODPATH}/system/priv-app/SecurityCenter.apk
		echo -n  " [mi9游戏工具箱高级直]" >>$INSTALLER/module.prop
	...
	echo -n  " [mix3游戏工具箱]" >>$INSTALLER/module.prop
	 cp -f $INSTALLER/common/mix/SecurityCenter.apk ${MODPATH}/system/priv-app/SecurityCenter.apk
      ...
      if [ "$f" == "1" ]; then
    ui_print "  检测到您已安装mix3游戏工具箱请选择是否清除相关组件数据"
 ...
 if $FUNCTION; then  
    ...
    ui_print "  检测到您已安装MI9游戏工具箱请选择是否保守清除相关组件数据"
    ui_print "音量＋:清除(系统版本较低时推荐)"
    ui_print "音量－:不清除"
 if $FUNCTION; then  
    rm -f /data/system/package_cache/1/SecurityCenter-*
...
miuiver="`grep_prop ro.build.version.incremental`"
	notver=$miuiver
	getfilever 8.11.1 10.3.0 && notver=9.1.24
  ...
  cp -f $INSTALLER/common/$notver/PowerKeeper.apk ${MODPATH}/system/app/PowerKeeper/
  else
  ui_print "未安装ai预加载"
  fi
  ...
  cp -f $INSTALLER/common/$notver/DownloadProviderUi.apk ${MODPATH}/system/priv-app/DownloadProviderUi/
  else
...
ui_print "   本模块边缘手势已支持您的MIUI版本"
 ...
 ui_print "已选择荷塘背景"
  if $FUNCTION; then  
	cd "$INSTALLER/common/byGesture/bj"
	$zip -r ${MODPATH}/system/media/theme/default/com.android.systemui ./* >/dev/null
		ui_print "   [荷塘背景]安装成功。"
		echo -n "[荷塘背景]；" >>$INSTALLER/module.prop
 	z=1
  else
 ui_print "已选择蓝色"
 if $FUNCTION; then  
...
if $FUNCTION; then
 ...
 miuiver="`grep_prop ro.build.version.incremental`"
	notver=$miuiver
		getfilever 9.3.7 9.3.8 && notver=9.1.24
	if [ -d $INSTALLER/common/$notver ]; then
	ui_print "- 请按音量键选择是否安装[关屏动画] -"
	ui_print "   [音量+]：安装"
ui_print "   [音量-]：不安装"
if $FUNCTION; then
	cd "$INSTALLER/common/gp"
	$zip -r ${MODPATH}/system/framework/services.jar ./* >/dev/null
		ui_print "已安装。"
	else
	ui_print "未安装。"
	fi
